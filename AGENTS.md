...
- **"Address the next TODOs" ⇒ batch, don't bottleneck.** When the user asks to tackle the next TODO(s), default to picking a *handful* of orthogonal items and parallelizing them across subagents (using isolated worktrees when needed). Confirm *which* items with the user. **Each subagent marks its own item `[~]` on claim and `[x]` once done and verified**, so `TODO.md` stays an accurate live view of what is being worked on.
...
- **Track execution live in `EXPERIMENTS.md`:** add a `PLANNED` row when the experiment is defined; at launch set `server` and `RUNNING`; when execution ends set `COMPLETED`, `FAILED`, or `ABORTED`. The agent runs and monitors its experiments: check about hourly that a `RUNNING` one is actually computing, not just alive (see the header of `EXPERIMENTS.md`). These statuses describe execution only. Never populate analysis columns or infer findings as part of this lifecycle.
