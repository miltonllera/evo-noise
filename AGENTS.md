...
- **Agree every experiment name/id with the user during experiment definition.** Ask how to name each experiment as part of the project-definition interview; agents may suggest semantic ids, but must not choose or improvise the final id without the user's explicit confirmation. If an experiment is introduced later, get the same confirmation during its planning and before creating its config, TODO/index entry, results folder, or launch. Use a semantic id — two to four words saying what the experiment did (`field-sparsity`), never a bare number; a trailing number only marks a repeat of the same experiment. The same string names `configs/<id>.yaml`, `results/<id>/`, the `EXPERIMENTS.md` row, the analysis file and the report title (per [`EXPERIMENT_ANALYSIS.md`](md/instructions/EXPERIMENT_ANALYSIS.md)).
