...
AGENTS.md RESEARCH.md TODO.md EXPERIMENTS.md DIAGRAMS.md DONE.md   # top-level md
```
...
- **`RESEARCH.md`** — the project and scientific framing: research questions, technical specifications, current status, and the cross-experiment synthesis (what the completed experiments collectively say about the research questions — per-experiment analysis lives in `md/experiments/`, indexed in `EXPERIMENTS.md`, per [`EXPERIMENT_ANALYSIS.md`](EXPERIMENT_ANALYSIS.md)). As the project grows, new information goes into a specific `md/` file and is cross-referenced here.
...
- **Before every non-trivial commit, make sure the following are current:**
  ...
  - Plan diagrams: verify the `mermaid` diagrams in `RESEARCH.md`, `EXPERIMENTS.md`, and `TODO.md` still match the text and status of their files (see [`DIAGRAMS.md`](DIAGRAMS.md)).
...
- Keep RESEARCH.md lean: `md/` is the project's second brain for durable reference knowledge (code and experiment scripts docs, methodology, model/hyperparameter rationale, dataset descriptions, preprocessing, workflows, agent instructions), not results or data (those live in `results/` and `data/`). Record what's worth keeping long-term and consulting again, not transient or one-off detail; consult `md/` before re-deriving something that may already be documented. Put new detail in a specific `md/` file and cross-reference it in RESEARCH.md. Experiment results and analyses go to `md/experiments/` + `EXPERIMENTS.md` (see [`EXPERIMENT_ANALYSIS.md`](EXPERIMENT_ANALYSIS.md)), not RESEARCH.md.
  - Link syntax: always use plain markdown links (`[text](path)`) for cross-references — they behave the same in every agent (inert pointers, fetched on demand). Never use bare `@file.md` refs: in Claude Code they auto-load the target into context every session when the referencing file is in the AGENTS.md import chain, while Codex treats them as plain text.
...
- Each plan carries a standing `mermaid` diagram: the research plan in `RESEARCH.md`, the experiments map in `EXPERIMENTS.md`, and the implementation roadmap in `TODO.md` — drawn and maintained per [`DIAGRAMS.md`](DIAGRAMS.md).
...
- **Record each completed experiment's analysis as specified in [`EXPERIMENT_ANALYSIS.md`](EXPERIMENT_ANALYSIS.md):** one file per experiment in `md/experiments/`, a row in `EXPERIMENTS.md`, and RESEARCH.md updated only if the synthesis changes.
