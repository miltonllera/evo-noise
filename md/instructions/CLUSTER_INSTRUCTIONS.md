...
One doc per cluster: [`SLURM_INSTRUCTIONS.md`](SLURM_INSTRUCTIONS.md) holds scheduler-generic behaviour, and each cluster's doc holds only its own facts (host, partitions, GPU tags, limits). Adding a cluster means adding a file here and a row above — never editing another cluster's doc.

## True on every target

`<repo>` is the repository we're working in.

- **Commit and push locally first.** The remote pulls from the git remote, so anything uncommitted or unpushed isn't there. Unrelated dirty changes you didn't make: surface them, don't sweep them into a commit.
- **Pull on the remote, and stop at surprises.** Conflict, dirty tree, unexpected branch, diverged history → **STOP and ask the user.** Don't force, reset, or stash-discard.
...
- **Record the launch in `EXPERIMENTS.md`:** once the job is confirmed started, fill the experiment's row — `server` column `<target> (<job/session id>)`, finding column `RUNNING — launched <date>` (per [`EXPERIMENT_ANALYSIS.md`](EXPERIMENT_ANALYSIS.md)) — and commit it: the live record of what runs where, visible to every agent.
